class Sparkler
    ...
    if request.path_info =~ %r(^/feed/([\w\-\.]+)$)
      ...
      if feed
        ...
        if (userAgent = env["HTTP_USER_AGENT"]) && userAgent =~ %r(Sparkle/)
          save_params(request.params)
          save_param('appVersionShort', userAgent.split(' ').first.split('/').last)
        end
  ...
  def save_params(params)
    ...
    params.each do |property_name, value_name|
      save_param(property_name, value_name)
    end
  end

  def save_param(property_name, value_name)
